...
import { fetchMedia, refreshStatsAfterMutation } from "../services/media.js";
...
export async function submitBulkStatus(status: string): Promise<void> {
  ...
  store.set((prev) => ({ ...prev, bulkAction: "status" }));
  try {
    ...
    store.set((prev) => ({ ...prev, bulkAction: "" }));
  }
}
...
export async function submitBulkIncrement(): Promise<void> {
  ...
  store.set((prev) => ({ ...prev, bulkAction: "increment" }));
  try {
    const updates = await Promise.allSettled(
      ids.map((id) => {
...
export async function submitBulkDelete(): Promise<boolean> {
  ...
  store.set((prev) => ({ ...prev, bulkAction: "delete" }));
  try {
    let ok = 0;
    let fail = 0;
    const CHUNK = 500;
